...
fn main() {
    ...
    println!("Benchmarking Rust LTX operations ({} iterations, {} warmup)\n", iterations, warmup);
    println!("{:<30} {:<15} {:<15}", "Operation", "Time (ms)", "Throughput");
    println!("{}", "-".repeat(60));
    ...
    time_fn("Patchify 5D roundtrip", &mut || {
    ...
    time_fn("FP8 quantize (256x512)", &mut || {
        let _ = ltx_fp8::quantize_weight_to_fp8_per_tensor(&w);
    }, iterations, warmup);

    // Scheduler
    time_fn("Scheduler sigmas (n=50)", &mut || {
