use ltx_types::utils::{to_velocity, to_denoised, projection_coef};
...
fn test_to_velocity_sigma_zero() {
    ...
    assert_eq!(vel.isnan().any().double_value(&[]), 0.0, "to_velocity produced NaN at sigma=0");
    assert_eq!(vel.isinf().any().double_value(&[]), 0.0, "to_velocity produced Inf at sigma=0");
}
...
fn test_to_velocity_small_sigma() {
    ...
    assert!(vel.isfinite().all().double_value(&[]) > 0.0, "to_velocity overflow at small sigma");
}
...
fn test_to_denoised_roundtrip() {
    ...
    assert!(max_diff < 1e-4, "to_denoised roundtrip: max diff = {max_diff}");
}
...
fn test_projection_coef_finite() {
    ...
    assert!(coef.isfinite().all().double_value(&[]) > 0.0, "projection_coef produced NaN/Inf");
