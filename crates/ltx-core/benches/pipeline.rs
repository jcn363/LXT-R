...
use ltx_patchify::{patchify_4d, unpatchify_4d, patchify_5d, unpatchify_5d};
...
fn bench_patchify_4d(c: &mut Criterion) {
    ...
    group.bench_function("3ch 64x64", |b| {
        b.iter(|| patchify_4d(&x, 4))
    });
    let x = Tensor::randn([1, 16, 32, 32], (tch::Kind::Float, tch::Device::Cpu));
    group.bench_function("16ch 32x32", |b| {
        b.iter(|| patchify_4d(&x, 2))
    });
    group.finish();
}
...
fn bench_rope(c: &mut Criterion) {
    ...
    let (cos_full, sin_full) = precompute_freqs_cis(128, 512, 10000.0, RopeType::Interleaved, tch::Device::Cpu);
...
fn bench_sdpa(c: &mut Criterion) {
    ...
    let k = Tensor::randn([1, 8, 128, 128], (tch::Kind::Float, tch::Device::Cpu));
    let v = Tensor::randn([1, 8, 128, 128], (tch::Kind::Float, tch::Device::Cpu));
    let mask = Tensor::ones([128, 128], (tch::Kind::Bool, tch::Device::Cpu))
        .tril(0);
