...
pub fn show(ui: &mut egui::Ui, state: &AppState) {
    ui.horizontal(|ui| {
        let (label, tooltip, color) = match &state.inference {
            InferenceState::Idle => (
                "Ready".to_string(),
                "No generation in progress".to_string(),
                egui::Color32::GRAY,
            ),
            ...
            InferenceState::Decoding => (
                "Decoding…".to_string(),
                "Converting latent to pixel space".to_string(),
                egui::Color32::YELLOW,
            ),
            InferenceState::Done => (
                "Done".to_string(),
                "Generation complete — frames ready for preview and export".to_string(),
        ...
        if !state.frames_display.is_empty() {
            ui.label(format!(
                ...
                state.width,
                state.height,
                state.frames_display.len()
            ))
            .on_hover_text(format!(
                "Width × Height × Frames ({} total pixels)",
                state.width * state.height * state.frames_display.len() as i64
            ));
        } else {
