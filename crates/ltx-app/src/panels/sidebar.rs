...
pub fn show(ui: &mut egui::Ui, state: &mut AppState) {
    ...
    // Prompt
    ui.label("Prompt")
        .on_hover_text("Text description of the video to generate");
    ...
    ui.label("Model Weights")
        .on_hover_text("Transformer .safetensors checkpoint (omit for random init)");
    ui.horizontal(|ui| {
        ...
        ui.label(&label)
            .on_hover_text("Currently loaded weight file");
        if ui
            .button("Browse…")
            .on_hover_text("Select transformer weights file")
            .clicked()
        {
        ...
        if state.weights_path.is_some()
            && ui
                .button("×")
                .on_hover_text("Clear weights (revert to random init)")
                .clicked()
        {
    ...
    // Text Encoder
    ui.label("Text Encoder").on_hover_text(
        "SentencePiece tokenizer + text encoder weights (T5 or Gemma3) for prompt conditioning",
    );
    ui.horizontal(|ui| {
        ...
        ui.label(&label)
            .on_hover_text("SentencePiece tokenizer model (.model file)");
        if ui
            .button("Browse…")
            .on_hover_text("Select tokenizer model file")
            .clicked()
        {
            if let Some(path) = rfd::FileDialog::new()
    ...
    ui.horizontal(|ui| {
        ...
        ui.label(&label)
            .on_hover_text("Text encoder weights (.safetensors file)");
        if ui
            .button("Browse…")
            .on_hover_text("Select text encoder weights")
            .clicked()
        {
            if let Some(path) = rfd::FileDialog::new()
                .add_filter("SafeTensors", &["safetensors"])
    ...
    ui.label("Resolution").on_hover_text(
        "Latent-space dimensions (pixel = latent × scale factor, e.g. 16×32 = 512×1024)",
    );
    ui.horizontal(|ui| {
    ...
    ui.horizontal(|ui| {
        ui.label("Frames")
            .on_hover_text("Number of frames to generate");
    ...
    // Inference params
    ui.label("Steps")
        .on_hover_text("Number of denoising steps (more = better quality, slower)");
    ui.add(egui::Slider::new(&mut state.steps, 1..=100))
        .on_hover_text("Denoising steps: 5-10 for quick preview, 20-50 for quality");
    ui.label("CFG Scale")
        .on_hover_text("Classifier-free guidance strength (higher = follows prompt more)");
    ...
    // Scheduler
    ui.label("Scheduler")
        .on_hover_text("Noise schedule for diffusion denoising");
    ui.horizontal(|ui| {
    ...
    // Device
    ui.label("Device")
        .on_hover_text("Compute device for transformer inference");
    ui.horizontal(|ui| {
    ...
    let can_generate = matches!(
        state.inference,
        crate::state::InferenceState::Idle
            | crate::state::InferenceState::Done
            | crate::state::InferenceState::Error(_)
    );
    if can_generate {
        if ui
    ...
    if !state.frames_display.is_empty() {
        ui.separator();
        ui.label("Export")
            .on_hover_text("Save generated frames to disk");
        ui.horizontal(|ui| {
            if ui
                .button("Save PNGs")
                .on_hover_text("Save each frame as a separate PNG file")
                .clicked()
            {
            ...
            if ui
                .button("Save Video")
                .on_hover_text("Save as MP4 video (H.264 encoded, 8fps)")
                .clicked()
            {
                if let Some(path) = rfd::FileDialog::new()
                    .add_filter("MP4", &["mp4"])
            ...
            if ui
                .button("Save GIF")
                .on_hover_text("Save as animated GIF (256×256, lanczos scaling)")
                .clicked()
            {
                if let Some(path) = rfd::FileDialog::new()
                    .add_filter("GIF", &["gif"])
