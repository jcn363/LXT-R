...
impl EmbeddingsConnector {
    ...
    pub fn concatenate(&self, text_embeds: &Tensor, vision_embeds: &Tensor) -> Tensor {
        Tensor::cat(&[text_embeds, vision_embeds], 1)
    }

    /// Create an attention mask that blocks cross-modal attention.
    pub fn create_cross_mask(&self, text_len: i64, vision_len: i64, device: tch::Device) -> Tensor {
