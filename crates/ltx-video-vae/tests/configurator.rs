...
fn test_encoder_block_descs_types() {
    ...
    assert!(matches!(descs[2].kind, EncoderBlockKind::ChannelChangeDownsample));
    assert!(matches!(descs[3].kind, EncoderBlockKind::ResBlocks(3)));
    assert!(matches!(descs[4].kind, EncoderBlockKind::DownsampleConv));
    assert!(matches!(descs[5].kind, EncoderBlockKind::ChannelChangeDownsample));
...
fn test_encoder_architecture_totals() {
    ...
    let total_resblocks: i64 = descs.iter().map(|d| match d.kind {
    ...
    let downsamples = descs.iter().filter(|d| !matches!(d.kind, EncoderBlockKind::ResBlocks(_))).count();
